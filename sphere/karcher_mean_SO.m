function [mu] = karcher_mean_SO(X, varargin)
...
while true
    ...
    for i = 1 : N
        gradient = gradient + W(i) * logmap_SO(mu, X(:, i));
    end
    gradient = 2 * gradient/ N;
    mu = expmap_SO(mu, lambda * gradient);
    
    %   ...Calculate mean square error...   %
    error = 0;
    for i = 1 : N
        error = error + W(i) * norm(logmap_SO(mu, X(:,i)),'fro') ^ 2;
    end
    error = error/ N;
